...
- ✅ 结果在编译期可确定 → 用编译期计算
- ✅ 配置、查找表、静态数据 → 用编译期生成
- ✅ 运行时输入但范围确定 → 可以编译期生成查找表
- ❌ 输入范围不确定或范围过大 → 只能运行时计算

**核心结论：**  
> 编译期计算是用编译时间和代码空间换运行时速度。

---

## 第 13 页｜工程实践建议

1. **使用 constexpr实现编译期求值**
   - 代替宏定义的常量
   - 代替魔法数字
   - 编译期可确定的计算（配置、查找表、数学运算）
   - 既可编译期计算也可运行时计算，灵活性强
...
### 本篇总结：编译期计算的价值
模板元编程不只是技巧，而是打开了编译期计算的大门，但是实现方式晦涩。constexpr 让编译期计算更加简单直接。
模板元编程同样用于类型萃取，但是同样实现方式晦涩一些，未来要介绍的 “概念concept”，会让类型萃取变得更加简单。
...
我们将深入探讨 C++ 模板元编程中的类型萃取技术（type traits），以及 C++20 引入的 Concept 如何让类型约束变得更加简洁直观。
